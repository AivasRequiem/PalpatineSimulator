...
void UForceHandComponent::TurnBackCollision()
{
	if (MoveTarget)
		MoveTarget->SetCollisionEnabled(PreviousCollisionEnabled);
}
...
void UForceHandComponent::ForcePullActivate()
{
	...
	const bool IsHit = UKismetSystemLibrary::BoxTraceMulti(this, HandController->GetPivotLocation(),
	                                                        HandController->GetPivotLocation() + HandController->CustomPivotComponent->GetForwardVector() * 1500.0f, FVector(10, 10, 10),
	                                                        HandController->CustomPivotComponent->GetComponentRotation(),
	                                                        UEngineTypes::ConvertToTraceType(ECC_GameTraceChannel1), true,
	                                                        ActorsToIgnore, EDrawDebugTrace::Persistent, HitResults,
	                                                        true);
	...
	if (!HitResults.IsEmpty())
	{
		for (FHitResult HitResult : HitResults)
		{
			if (HitResult.Component->IsAnySimulatingPhysics())
			{
				...
				PreviousCollisionEnabled = MoveTarget->GetCollisionEnabled();
				MoveTarget->SetCollisionEnabled(ECollisionEnabled::NoCollision);
				
				MoveTimeline.SetPlayRate(0.3f);
				MoveTimeline.PlayFromStart();

				GetWorld()->GetTimerManager().SetTimerForNextTick(this, &UForceHandComponent::TurnBackCollision);
